...
const renderBackItem = (classes, menuConfig, menuDispatch) => {
  ...
  const item = getItem(menuConfig.settings, menuConfig.parentSettingsKeyArr, true);
  ...
  return (
    <Fragment>
      ...
      >
        ...
        { (menuConfig.variant !== menuTypes.MINIMIZED || menuConfig.open) && (<ListItemText primary={menuConfig.parentSettingsCaption} />)}
      </ListItem>
      <Divider />
    </Fragment>
  );
...
const MenuBackPanel = ({classes}) => {
  ...
  return (
    <Fragment>
      ...
      { (Array.isArray(menuConfig.parentSettingsKeyArr) && menuConfig.parentSettingsKeyArr.length > 0) && 
