...
import type {Localization} from '@material-ui/core/locale';
...
const getRows = (currentLocale:string) => [
...
function SimpleTable({props, currentLocale}:{props:TableProps, currentLocale: string}) {
...
const Page = () => {
  ...
  const localLocales: ILocales = locales;
  ...
  return (
    <section>
      ...
      <ThemeProvider theme={(outerTheme) => createMuiTheme(outerTheme, localLocales[localeConfig.currentLocaleMui])}>
        <TablePagination
            count={5}
            rowsPerPage={10}
            page={0}
